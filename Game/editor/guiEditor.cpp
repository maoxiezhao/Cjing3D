...
namespace Cjing3D {
	namespace Editor {
		...
		std::unique_ptr<IMGUIStage> mImGuiStage = nullptr;
		...
		void InitializeEditor()
		{
			...
			mImGuiStage = std::make_unique<IMGUIStage>(GlobalGetSubSystem<GUIStage>().GetGUIRenderer());
			mImGuiStage->Initialize();

			mImGuiStage->RegisterCustomWindow(ShowMainMenuBar);
			mImGuiStage->RegisterCustomWindow(ShowAboutWindow);
			mImGuiStage->RegisterCustomWindow(UpateMousePicking);

			InitializeEditorHierarchy(*mImGuiStage);
			InitializeEditorProfiler(*mImGuiStage);
			InitializeEditorLight(*mImGuiStage);
			InitializeEditorGUI(*mImGuiStage);
			InitializeEditorAnimation(*mImGuiStage);
			InitializeEditorRender(*mImGuiStage);
			InitializeEditorSound(*mImGuiStage);
			InitializeEditorParticle(*mImGuiStage);
#endif
		}
