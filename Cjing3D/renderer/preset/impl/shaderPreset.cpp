...
namespace {
	...
	void LoadVertexShaders(std::shared_ptr<InputLayout>* inputLayouts, std::shared_ptr<Shader>* shaders)
	{
		...
		const std::string shaderPath = resourceManager.GetStandardResourceDirectory(Resource_Shader);
		{
			...
			if (!Renderer::GetDevice().CheckGraphicsFeatureSupport(GraphicsFeatureSupport::VIEWPORT_AND_RENDERTARGET_ARRAYINDEX_WITHOUT_GS)) {
				...
				shaders[VertexShaderType_ShadowCube] = LoadShader(SHADERSTAGES_VS, shaderPath + "cubeShadowVS.cso");
			}
	...
	void LoadPixelShaders(std::shared_ptr<Shader>* shaders)
	{
		...
		{
			// object
			shaders[PixelShaderType_Object_Forward] = LoadShader(SHADERSTAGES_PS, shaderPath + "objectForwardPS.cso");
			shaders[PixelShaderType_Object_Forward_Transparent] = LoadShader(SHADERSTAGES_PS, shaderPath + "objectForwardTransparentPS.cso");
			shaders[PixelShaderType_Object_AlphaTest] = LoadShader(SHADERSTAGES_PS, shaderPath + "objectAlphaTestPS.cso");
			shaders[PixelShaderType_Object_TiledForward] = LoadShader(SHADERSTAGES_PS, shaderPath + "objectTiledForwardPS.cso");
			shaders[PixelShaderType_Object_TiledForward_Transparent] = LoadShader(SHADERSTAGES_PS, shaderPath + "objectTiledForwardTransparentPS.cso");

			// full screen ps
			shaders[PixelShaderType_FullScreen] = LoadShader(SHADERSTAGES_PS, shaderPath + "screenPS.cso");
			// image ps
			shaders[PixelShaderType_Image] = LoadShader(SHADERSTAGES_PS, shaderPath + "imagePS.cso");
			// full screen ps
			shaders[PixelShaderType_Sky] = LoadShader(SHADERSTAGES_PS, shaderPath + "skyPS.cso");
			// color pos
			shaders[PixelShaderType_PosColor] = LoadShader(SHADERSTAGES_PS, shaderPath + "posColorPS.cso");
		}
	}
	...
	void LoadComputeShaders(std::shared_ptr<Shader>* shaders)
	{
		...
		const std::string shaderPath = resourceManager.GetStandardResourceDirectory(Resource_Shader);
		{
			shaders[ComputeShaderType_Tonemapping] = LoadShader(SHADERSTAGES_CS, shaderPath + "toneMapping.cso");
			shaders[ComputeShaderType_MipmapGenerate] = LoadShader(SHADERSTAGES_CS, shaderPath + "mipmapGenerateCS.cso");
			shaders[ComputeShaderType_MipmapCubeGenerate] = LoadShader(SHADERSTAGES_CS, shaderPath + "mipmapGenerateCubeCS.cso");
			shaders[ComputeShaderType_MipmapGenerateUnorm] = LoadShader(SHADERSTAGES_CS, shaderPath + "mipmapGenerateUnormCS.cso");
			shaders[ComputeShaderType_MipmapCubeGenerateUnorm] = LoadShader(SHADERSTAGES_CS, shaderPath + "mipmapGenerateCubeUnormCS.cso");
			shaders[ComputeShaderType_FXAA] = LoadShader(SHADERSTAGES_CS, shaderPath + "fxaaCS.cso");
			shaders[ComputeShaderType_Skinning] = LoadShader(SHADERSTAGES_CS, shaderPath + "skinningCS.cso");
			shaders[ComputeShaderType_TiledFrustum] = LoadShader(SHADERSTAGES_CS, shaderPath + "tileFrustumCS.cso");
			shaders[ComputeShaderType_LightTiledCulling] = LoadShader(SHADERSTAGES_CS, shaderPath + "lightTiledCullingCS.cso");
			shaders[ComputeShaderType_LinearDepth] = LoadShader(SHADERSTAGES_CS, shaderPath + "linearDepthCS.cso");
			shaders[ComputeShaderType_SSAO] = LoadShader(SHADERSTAGES_CS, shaderPath + "ssaoCS.cso");
			shaders[ComputeShaderType_GaussianBlur] = LoadShader(SHADERSTAGES_CS, shaderPath + "gaussianBlurCS.cso");
			shaders[ComputeShaderType_BilateralBlur] = LoadShader(SHADERSTAGES_CS, shaderPath + "bilateralBlurCS.cso");
			shaders[ComputeShaderType_BilateralUpsample] = LoadShader(SHADERSTAGES_CS, shaderPath + "bilateralUpsampleCS.cso");
		}
	}
}
