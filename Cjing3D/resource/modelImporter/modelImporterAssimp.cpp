...
namespace ModelImporter {
	namespace {
		...
		void ProcessNode(SystemContext& SystemContext, const aiScene * aScene, aiNode * aNode)
		{
			// if is root node
	...
	void ImporrModelAssimp(const std::string& fileName, SystemContext& systemContext)
	{
		bool result = false;
		...
		const aiScene* scene = importer.ReadFile(fileName, importFlag);
		if (scene != nullptr)
		{
			ProcessNode(systemContext, scene, scene->mRootNode);
			importer.FreeScene();
		}
	}
}
